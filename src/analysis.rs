...
pub fn find_blunders(
    ...
    for (move_num, chess_move) in result.moves.iter().take(num_moves).enumerate() {
        ...
        let (_, eval_before) =
            get_evaluation_from_position(&engine_1, &chess_move.fen_before, DEFAULT_DEPTH)?;
        let (_, eval_after) =
            get_evaluation_from_position(&engine_2, &chess_move.fen_after, DEFAULT_DEPTH)?;
...
fn get_evaluation_from_position(
    engine: &Engine,
    fen: &str,
    depth: usize,
...
fn get_best_move(analysis: &str) -> String {
    let re = Regex::new("bestmove ([-a-z0-9]*)").unwrap();
...
fn get_evaluation_from_string(analysis: &str) -> Evaluation {
    let re = Regex::new("score (cp|mate) ([-0-9]*)").unwrap();
    let captures = re.captures_iter(&analysis);
    let capture = captures.last().unwrap();
