...
fn main() -> Result<()> {
    ...
    match args.command {
        Command::Scan(opts) => scan(database, opts),
        Command::Show => show_blunders(database),
    }
}

fn scan(database: Database, opts: ScanOpts) -> Result<()> {
...
fn run_scans(mut database: Database, opts: ScanOpts, games: Vec<GameInfo>) -> Result<()> {
